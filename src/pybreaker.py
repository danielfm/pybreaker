...
__all__ = (
    ...
    'CircuitMemoryStorage', 'CircuitRedisStorage', 'CircuitRedisClusterStorage', 'STATE_OPEN', 'STATE_CLOSED',
    'STATE_HALF_OPEN',)
...
class CircuitRedisClusterStorage(CircuitBreakerStorage):
    """
    Implements a `CircuitBreakerStorage` using redis.
    """
    ...
    def __init__(self, state, redis_object, namespace=None, fallback_circuit_state=STATE_CLOSED):
        ...
        # Module does not exist, so this feature is not available
        if not HAS_REDIS_SUPPORT:
            raise ImportError("CircuitRedisClusterStorage can only be used if the required dependencies exist")

        super(CircuitRedisClusterStorage, self).__init__('redis')

        try:
            self.RedisError = __import__('redis').exceptions.RedisError
        except ImportError:
            # Module does not exist, so this feature is not available
            raise ImportError("CircuitRedisClusterStorage can only be used if 'redis' is available")
